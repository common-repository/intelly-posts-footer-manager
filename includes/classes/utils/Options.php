...
class IPFM_Options {
    ...
    protected function getClassSession($class, $key, $default=FALSE) {
        ...
        if($result!==$default) {
            ...
            $result=$ipfm->Utils->jsonToClass($result, $class);
            if($result===FALSE) {
                $result=$default;
            }
        }
    ...
    protected function setClassSession($class, $key, $value) {
        global $ipfm;
        if(is_object($value)) {
            $value=$ipfm->Utils->classToJson($value);
        }
    ...
    protected function getClassOption($class, $key) {
        ...
        if($result===FALSE) {
            $result=$ipfm->Dao->Utils->newDomainClass($class);
        }
        return $result;
    }
    protected function setClassOption($key, $value, $overwrite=FALSE) {
        ...
        if(!$overwrite) {
            ...
            $data=$ipfm->Utils->parseArgs($value, $previous);
            $data=$ipfm->Utils->classToJson($data);
            $value=$data;
        }
    ...
    public function getTrackingLastSend() {
        return $this->getOption('TrackingLastSend['.IPFM_PLUGIN_SLUG.']', 0);
    }
    public function setTrackingLastSend($value) {
        $this->setOption('TrackingLastSend['.IPFM_PLUGIN_SLUG.']', $value);
    }
    public function getPluginInstallDate() {
        return $this->getOption('PluginInstallDate['.IPFM_PLUGIN_SLUG.']', 0);
    }
    public function setPluginInstallDate($value) {
        $this->setOption('PluginInstallDate['.IPFM_PLUGIN_SLUG.']', $value);
    }
    public function getPluginUpdateDate() {
        return $this->getOption('PluginUpdateDate['.IPFM_PLUGIN_SLUG.']', 0);
    }
    public function setPluginUpdateDate($value) {
        $this->setOption('PluginUpdateDate['.IPFM_PLUGIN_SLUG.']', $value);
    }
    ...
    private function pushGenericMessage($type, $message, $v1=NULL, $v2=NULL, $v3=NULL, $v4=NULL, $v5=NULL) {
        ...
        $m=$ipfm->Lang->L($message, $v1, $v2, $v3, $v4, $v5);
        $exists=FALSE;
        foreach($array as $v) {
            if($v==$m) {
                $exists=TRUE;
    ...
    private function writeGenericMessages($type, $clean=TRUE) {
        ...
        if(is_array($array) && count($array)>0) {
            $result=TRUE;
            $text="<p>".$ipfm->Utils->implode("", "", "<br/>\n", $array)."</p>";
            $ipfm->Ui->alert($type, $text);
            ?>
        <?php }
        if($clean) {
    ...
    //ShowLicenseData
    public function isShowLicenseData() {
        return $this->getOption('ShowLicenseData', FALSE);
    }
    public function setShowLicenseData($value) {
        $this->setOption('ShowLicenseData', $value);
    }
    //License
