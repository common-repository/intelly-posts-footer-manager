...
class IPFM_Utils {
    ...
    function inArray($needle, $haystack) {
        ...
        if(count($haystack)==0 || count($needle)==0) {
            return FALSE;
        }
    ...
    function query($query, $options=NULL) {
        ...
        $result=$ipfm->Options->getCache($key);
        if (!is_array($result) || count($result) == 0) {
            ...
            switch ($query) {
                ...
                case IPFM_QUERY_TAGS:
                    ...
                    break;
                case IPFM_QUERY_TAXONOMIES_OF_TYPE:
                    $taxonomy=$options['type'];
                    $args=array('hide_empty'=>FALSE);
                    $q=get_terms($taxonomy, $args);
                    if (empty($q)) {
                    ...
                    $this->_taxonomyType=$taxonomy;
                    $function=array($this, 'getTermLink');
                    break;
            }
            ...
            } elseif ($query==IPFM_QUERY_POST_TYPES) {
                $result=array();
                $result['post']='Post';
                $result['page']='Page';
                $result=$ipfm->Utils->toFormatListArrayFromListObjects($result, FALSE, '{text} ({id})');
            } elseif($query==IPFM_QUERY_TAXONOMY_TYPES) {
                global $wp_taxonomies;
                foreach($wp_taxonomies as $k=>$v) {
                    $isPublic=$ipfm->Utils->bget($v, 'public');
                    if($isPublic && $k!='post_format') {
                        $name=$ipfm->Utils->get($v, 'labels.singular_name');
                        if($options['type']!='') {
            ...
            $ipfm->Options->setCache($key, $result);
        }

        if ($options['all']) {
            $first=array();
            $first[]=array('id'=>-1, 'name'=>'['.$ipfm->Lang->L('All').']', 'url'=>'');
            $result=array_merge($first, $result);
        }
        if ($options['select']) {
            $first=array();
            $first[]=array('id'=>0, 'name'=>'['.$ipfm->Lang->L('Select').']', 'url'=>'');
            $result=array_merge($first, $result);
        }
    ...
    public function formatDatetime($time='now') {
        return $this->formatCustomDate($time, IPFM_Utils::FORMAT_DATETIME);
    }
    public function formatCompactDatetime($time='now') {
        return $this->formatCustomDate($time, IPFM_Utils::FORMAT_COMPACT_DATETIME);
    }
    public function formatDate($time='date') {
        return $this->formatCustomDate($time, IPFM_Utils::FORMAT_DATE);
    }
    ...
    public function formatTime($time='now') {
        return $this->formatCustomTime($time, IPFM_Utils::FORMAT_TIME);
    }
    public function formatSqlDatetime($time='now') {
        return $this->formatCustomDate($time, IPFM_Utils::FORMAT_SQL_DATETIME);
    }
    public function formatSqlDate($time='date') {
        return $this->formatCustomDate($time, IPFM_Utils::FORMAT_SQL_DATE);
    }
    public function formatSqlTime($time='now') {
        return $this->formatCustomTime($time, IPFM_Utils::FORMAT_SQL_TIME);
    }
    ...
    public function parseDateToArray($date) {
        ...
        if($ipfm->Utils->endsWith($date, 'AM')) {
            $date=substr($date, 0, strlen($date)-2);
            $date=trim($date);
        } elseif($ipfm->Utils->endsWith($date, 'PM')) {
            $date=substr($date, 0, strlen($date)-2);
            $date=trim($date);
            $pm=TRUE;
        }
        ...
        if(count($date)==1) {
            ...
            $result=$ipfm->Utils->parseArgs($a1, $a2);
        }
    ...
    public function parseDateToTime($date) {
        ...
        $a=$ipfm->Utils->parseArgs($result, $defaults);
    ...
    function toListArrayFromClass($array, $id=FALSE, $value=FALSE) {
        ...
        if($array!==FALSE && count($array)>0) {
            foreach($array as $k=>$v) {
                if($id!==FALSE) {
                    $k=$ipfm->Utils->get($v, $id);
                }
                if($value!==FALSE) {
                    $v=$ipfm->Utils->get($v, $value);
                }
        ...
        foreach($array as $instance) {
            ...
            foreach($colors as $color=>$when) {
                ...
                foreach($when['conditions'] as $conditionKey=>$conditionValue) {
                    $conditionValue=$ipfm->Utils->toArray($conditionValue);
    ...
    public function getConstants($class, $prefix, $reverse=FALSE) {
        ...
        if(!class_exists($class)) {
            $class=IPFM_PLUGIN_PREFIX.$class;
        }
    ...
    function remotePost($action, $data = '') {
        ...
        $data['secret']='WYSIWYG';
        $response = wp_remote_post(IPFM_INTELLYWP_ENDPOINT.'?iwpm_action='.$action, array(
            'method'=>'POST'
            , 'timeout'=>20
            , 'redirection'=>5
            , 'httpversion'=>'1.1'
            , 'blocking'=>TRUE
            , 'body'=>$data
            , 'user-agent'=>IPFM_PLUGIN_NAME.'/'.IPFM_PLUGIN_VERSION.'; '.get_bloginfo('url')
        ));
        $data = json_decode(wp_remote_retrieve_body($response), TRUE);
        if (is_wp_error($response) || wp_remote_retrieve_response_code($response) != 200
            || !isset($data['success']) || !$data['success']
        ) {
            $ipfm->Log->error('ERRORS SENDING REMOTE-POST ACTION=%s DUE TO REASON=%s', $action, $response);
            $data = FALSE;
        } else {
            $ipfm->Log->debug('SUCCESSFULLY SENT REMOTE-POST ACTION=%s RESPONSE=%s', $action, $data);
        }
        return $data;
    }
    function remoteGet($uri, $options) {
        ...
        $ipfm->Log->debug('REMOTEGET: URI=%s', $uri);
        $response=FALSE;//wp_remote_get($uri, $args);
        if ($response!==FALSE && !is_wp_error($response)) {
            // decode the license data
            $body=wp_remote_retrieve_body($response);
            if($body!==FALSE && $body!='') {
                    $ipfm->Log->debug('REMOTEGET: RETRIEVEBODY=%s', $body);
                    $result=json_decode($body);
                    if(!$result) {
                        $ipfm->Log->error('REMOTEGET: UNDECODABLE BODY');
                        if(strpos($body, 'debug')!==FALSE || strpos($body, 'fatal')!==FALSE || strpos($body, 'warning')!==FALSE || strpos($body, 'error')!==FALSE) {
                            $ipfm->Options->pushErrorMessage('XdebugException');
                        }
                    }
                } else {
                    $ipfm->Log->debug('REMOTEGET: RETRIEVEBODY ERROR');
                    $result=FALSE;
                }
        } else {
            $ipfm->Log->debug('REMOTEGET: RESULT=ERROR');
        }
        if($result===FALSE) {
            $ipfm->Log->debug('file_get_contents: URI=%s', $uri);
            ...
            $ipfm->Log->debug('file_get_contents: RETRIEVEBODY=%s', $body);
            if($body!==FALSE && $body!='') {
                $ipfm->Log->debug('REMOTEGET: RETRIEVEBODY=%s', $body);
                $result=json_decode($body);
                if(!$result) {
    ...
    function isPluginPage() {
        ...
        $result=($this->startsWith($page, IPFM_PLUGIN_SLUG));
        return $result;
    }
    function isQsNull($v) {
    ...
    function jsonToClass($json, $class) {
        global $ipfm;
        $instance=$ipfm->Dao->Utils->getClass($class);
        if($instance=='') {
        ...
        if($ipfm->Utils->isArray($json) && !$ipfm->Utils->isAssociativeArray($json)) {
            $match=FALSE;
            $result=array();
            foreach($json as $v) {
        ...
        } elseif($ipfm->Utils->isAssociativeArray($json) || is_object($json)) {
    ...
    private function jsonToInstance($json, $class) {
        ...
        $match=FALSE;
        $result=FALSE;
        $columns=$ipfm->Dao->Utils->getAllColumns($class);
        $instance=$ipfm->Dao->Utils->getClass($class);
        ...
        foreach($json as $property=>$value) {
            if(isset($columns[$property])) {
                ...
                if(isset($column['ui-type']) && $column['ui-type']=='array') {
                    ...
                    $value=$ipfm->Dao->Utils->decode($class, $property, $value);
                }
            }
            if($ipfm->Utils->set($instance, $property, $value)) {
                $match=TRUE;
            }
        }
        if($match) {
    ...
    public function decodeData($data=array()) {
        ...
        switch (strtolower($data['dataType'])) {
            case 'array':
                ...
                break;
            case 'class':
                if(class_exists($data['dataClass']) && $this->startsWith($data['dataClass'], IPFM_PLUGIN_PREFIX)) {
    ...
    function cqs($class, $prefix='') {
        global $ipfm;
        $result=$ipfm->Dao->Utils->qs($class, $prefix);
        return $result;
    }
    ...
    public function mail($to, $subject, $body, $options=array()) {
        ...
        if($options['footer'] && $options['html']) {
            $body .= '<br><hr><a href="'.IPFM_WORDPRESS_SITE.'"><img src="'.IPFM_SITE_IMAGES_URI.'logos/logo-mail.png" /></a>';
        }
        $result=FALSE;
    ...
    public function sortOptions_Compare($o1, $o2) {
        global $ipfm;
        $v1=$ipfm->Utils->get($o1, 'text', FALSE);
        if($v1===FALSE) {
            $v1=$ipfm->Utils->get($o1, 'name', FALSE);
        }
        $v2=$ipfm->Utils->get($o2, 'text', FALSE);
        if($v2===FALSE) {
            $v2=$ipfm->Utils->get($o2, 'name', FALSE);
        }

        //to order properly
        if($ipfm->Utils->startsWith($v1, '[')) {
            $v1=' '.$v1;
        }
        if($ipfm->Utils->startsWith($v2, '[')) {
            $v2=' '.$v2;
        }
    ...
    public function cURL($uri, $method, $options=array()) {
        ...
        try {
            ...
            if($errorCode==0 && isset($info['http_code'])) {
                ...
                } elseif(is_string($response) && $ipfm->Utils->startsWith($response, 'No site configured at this address')) {
                    $errorCode=-1;
                    if($errorDescription=='') {
                        $errorDescription=$response;
                    }
