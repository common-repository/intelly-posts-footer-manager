...
class IPFM_Tracking {
    public function __construct() {
        add_action('ipfm_weekly_scheduled_events', array($this, 'sendTracking'));
    }
    ...
    public function getData() {
        ...
        $post_types=$ipfm->Utils->query(IPFM_QUERY_POST_TYPES);
        $data=array();
        ...
        $data=array();
        $result['iwpm_plugin_name']=IPFM_PLUGIN_SLUG;
        $result['iwpm_plugin_version']=IPFM_PLUGIN_VERSION;
        $result['iwpm_plugin_data']=$data;
        $result['iwpm_plugin_install_date']=$ipfm->Options->getPluginInstallDate();
        $result['iwpm_plugin_update_date']=$ipfm->Options->getPluginUpdateDate();

        $result['iwpm_tracking_enable']=$ipfm->Options->isTrackingEnable();
        $result['iwpm_logger_enable']=$ipfm->Options->isLoggerEnable();
        $result['iwpm_feedback_email']=$ipfm->Options->getFeedbackEmail();
        return $result;
    }
    ...
    public function sendTracking($override=FALSE) {
        ...
        $result=-1;
        if(!$override && !$ipfm->Options->isTrackingEnable())
            return $result;

        // Send a maximum of once per week
        $last_send=$ipfm->Options->getTrackingLastSend();
        ...
        $data=$ipfm->Utils->remotePost('usage', $this->getData());
        if($data) {
            $result=intval($data['id']);
            $ipfm->Options->setTrackingLastSend(time());
        }
        return $result;
    }
}