...
class IPFM_DaoUtils {
    ...
    private function getColumnCreationSql($table, $primary, $name, $column) {
        ...
        $sql=$ipfm->Dao->encodeColumn($table, $column['column'])." ".$type;

        if(isset($column['len'])) {
            $sql.="(".$column['len'].")";
        }
        if(isset($column['default']) && !$ipfm->Utils->startsWith($column['default'], '{')) {
	...
	$result=FALSE;
        foreach ($this->tables as $k=>$v) {
            ...
            $sql.="\n);";
            $ipfm->Log->debug('CREATING TABLE %s...', $table);
            $ipfm->Log->debug('SCRIPT: %s', $sql);
    ...
    function loadClass($prefix, $root, $file) {
        ...
        if($file!=NULL && strlen($source)>0) {
                        ...
                        switch ($p) {
                            case $patternClass:
                                if(is_array($annotations)!==FALSE) {
                                    ...
                                    $row=explode(' ', $row);
                                    $class=$row[0];
                                    $data=$ipfm->Utils->merge(TRUE, $data, $annotations);
                                    $data['class']=$class;
                                } else {
                            ...
                            case $patternVar:
                                if(is_array($annotations)!==FALSE) {
                                    ...
                                    $v=$ipfm->Utils->get($annotations, 'primary', FALSE);
                                    $v=$ipfm->Utils->isTrue($v);
                                    $annotations['primary']=$v;
                                    $v=$ipfm->Utils->get($annotations, 'required', FALSE);
                                    $v=$ipfm->Utils->isTrue($v);
            ...
            if(isset($data['class']) && $data['class']!='') {
                ...
                $class=str_replace(IPFM_PLUGIN_PREFIX, '', $class);
                $this->data[$class]=$data;
                $result=TRUE;
            }
        }
    ...
    public function getColumns($class, $options=array()) {
        ...
        if(strpos($class, 'Search')!==FALSE) {
            ...
            if (isset($this->data[$domainClass])) {
                ...
                $patterns=$ipfm->Dao->getSearchPatterns();
                foreach ($result as $name=>$value) {
                    foreach ($patterns as $p) {
                        if ($p=='' || $ipfm->Utils->endsWith($name, $p)) {
                            $k=$name;
                            if (strlen($p)>0) {
                            ...
                            if(isset($defaults[$k])) {
                                ...
                                $array=$defaults[$k];
                                unset($array['default']);
                                $value=$ipfm->Utils->parseArgs($value, $array);
                                switch (strtolower($p)) {
                                    case 's':
                                    case 'Ids':
            ...
            foreach($result as $k=>$v) {
                if(isset($v['ui-type']) && $v['ui-type']=='pointer') {
                    if(strtolower($k)!='id' && $ipfm->Utils->endsWith($k, 'Id')) {
                        ...
                        if($options['includeNested']) {
                            ...
                            if($columns!==FALSE && count($columns)>0) {
                                $kk=$ipfm->Utils->lowerUnderscoreCase($k);
                                $kk=explode('_', $kk);
                                $alias='';
                                foreach($kk as $t) {
                                    $alias.=substr($t, 0, 1);
        ...
        $includes=$ipfm->Utils->toArray($options['includeUiTypes']);
        $excludes=$ipfm->Utils->toArray($options['excludeUiTypes']);
        if(count($includes)>0 || count($excludes)>0) {
            foreach($result as $k=>$v) {
                $include=(isset($v['ui-type']) && $v['ui-type']!='');
                if($include && !$ipfm->Utils->inArray($v['ui-type'], $includes)) {
                    $include=FALSE;
                }
                if($include && $ipfm->Utils->inArray($v['ui-type'], $excludes)) {
                    $include=FALSE;
                }
        ...
        if(is_array($result)) {
            ...
            $ipfm->Log->error('NO COLUMNS FOUND FOR CLASS=[%s]', $class);
        }
        ...
        if($result===FALSE) {
            ...
            foreach($fields as $k=>$v) {
                if(isset($v['default']) && $v['default']!=='') {
                    $v=$v['default'];
                    if(!$ipfm->Utils->startsWith($v, '{')) {
                        $result[$k]=$v;
                    }
                    /*if($v=='currentCallCenterId') {
                        $u=$ec->Session->getUser();
                        $v=$u->id;
                        if($v>0 && $u->userRight==IPFM_UserConstants::USER_RIGHT_CALL_CENTER) {
                            $result[$k]=$v;
                        }
                    } elseif($v=='currentAgentId') {
                        $u=$ec->Session->getUser();
                        $v=$u->id;
                        if($v>0 && ($u->userRight==IPFM_UserConstants::USER_RIGHT_AGENT || $u->userRight==IPFM_UserConstants::USER_RIGHT_AGENT_MANAGER)) {
                            $result[$k]=$v;
                        }
            ...
            foreach($columns as $name=>$column) {
                if(isset($column['ui-type']) && $column['ui-type']!='') {
                    ...
                    if($v!==FALSE || $allowEmpty) {
                        ...
                        if($ipfm->Utils->set($instance, $name, $value)) {
                            if($value!=='' || !$allowEmpty) {
                                $match=TRUE;
                            }
                        }
    ...
    public function decode($class, $name, $value) {
        ...
        $name=str_replace(IPFM_PLUGIN_PREFIX, '', $name);
        ...
        if($column && isset($column['type'])) {
            switch (strtolower($column['type'])) {
                ...
                case 'numeric':
                    if(is_numeric($value)) {
                        $value=$ipfm->Utils->parseNumber($value);
                    } else {
                ...
                case 'date':
                case 'time':
                    $value=$ipfm->Utils->parseDateToTime($value);
                    break;
                case 'array':
                    $value=$ipfm->Utils->dbarray($value, FALSE);
                    break;
                case 'json':
            ...
            if(isset($column['ui-type'])) {
                switch ($column['ui-type']) {
                    ...
                    case 'timer':
                        if($value!='') {
                            $value=$ipfm->Utils->parseTimer($value);
    ...
    public function isColumnText($class, $name) {
        ...
        $column=$this->getColumn($class, $name);
        if($column && isset($column['type'])) {
            if($ipfm->Utils->contains($column['type'], 'text') || $ipfm->Utils->contains($column['type'], 'char')) {
                $result=TRUE;
            }
        }
        return $result;
    ...
    public function encode($class, $name, $value, $quote) {
        ...
        if($column) {
            switch (strtolower($column['type'])) {
                case 'bool':
                    $value=($ipfm->Utils->isTrue($value) ? 1 : 0);
                    break;
                case 'int':
                case 'long':
                case 'float':
                ...
                case 'numeric':
                    ...
                    break;
                case 'datetime':
                    $value=$ipfm->Utils->formatSqlDatetime($value);
                    break;
                case 'date':
                    $value=$ipfm->Utils->formatSqlDate($value);
                    break;
                case 'time':
                    $value=$ipfm->Utils->formatSqlTime($value);
                    break;
                case 'array':
                    $array=$ipfm->Utils->dbarray($value, FALSE);
                    $value='';
                    foreach($array as $v) {
                        $value.=','.$v.',';
                    }
    ...
    public function newInnerClass($domainClass, $property, $innerClass) {
        ...
        $column=$ipfm->Dao->Utils->getColumn($domainClass, $property);
        if(isset($column['defaults']) && $column['defaults']!='') {
            $defaults=$ipfm->Utils->toArray($column['defaults']);
            foreach($defaults as $v) {
                $v=explode(':', $v);
                if(count($v)>1) {
                    $k=$v[0];
                    $v=$v[1];
                    $v=$ipfm->Utils->getConstant($innerClass, $k, $v);
                    $result->$k=$ipfm->Dao->Utils->decode($innerClass, $k, $v);
                }
            }
        }
        return $result;
    ...
    public function newDomainClass($class) {
        ...
        $columns=$ipfm->Dao->Utils->getAllColumns($result);
        foreach($columns as $k=>$v) {
            if(isset($v['default'])&& $v['default']!='') {
                $default=$v['default'];
                $default=str_replace('{url}', IPFM_BLOG_URL, $default);
                $default=str_replace('{domain}', $ipfm->Utils->trimHttp(IPFM_BLOG_URL), $default);
                $default=str_replace('{email}', IPFM_BLOG_EMAIL, $default);
                $default=$ipfm->Dao->Utils->decode($result, $k, $default);
                $ipfm->Utils->set($result, $k, $default);
            }

            if(isset($v['ui-type'])) {
                if($v['ui-type']=='pointer') {
                    $innerClass=$ipfm->Dao->Utils->getClass($v['rel']);
                    if ($v['instance']!='') {
                        $k=$v['instance'];
                        $v=$this->newInnerClass($result, $k, $innerClass);
                        $ipfm->Utils->set($result, $k, $v);
                    }
                } elseif($v['ui-type']=='toggle') {
                    $t=$ipfm->Utils->get($result, $k);
                    if(is_null($t)) {
                        $ipfm->Utils->set($result, $k, 0);
                    }
                }
            }
        }
    ...
    public function isColumnVisible($instance, $name) {
        ...
        if(isset($column['ui-visible']) && $column['ui-visible']!='') {
            ...
            foreach($conditions as $c) {
                ...
                $options=$options[1];
                $options=$ipfm->Utils->toArray($options);
